    ...
    <div>
      <el-table border :data="tableData.list">
        ...
        <el-table-column label="操作" min-width="120" fixed="right">
          <template slot-scope="scope">
            <span class="font-default pointer" @click="onModifyPrice(scope.row)">矿发煤价</span>
            <span
              v-if="scope.row.state === 50"
              class="font-default pointer ml-5"
              @click="onViewReason(scope.row)"
              >查看原因</span
            >
            <span
              class="font-default pointer"
              v-has="'sale:goods:edit'"
              @click="handleEdit(scope.row)"
              >编辑</span
            >
            <span
              class="font-red pointer ml-5"
              v-has="'sale:goods:del'"
...
export default {
  ...
  methods: {
    ...
    // 查看驳回原因
    async onViewReason(row) {
      let result = await axios.post(api.selectGoodsAuditList, { goodsId: row.goodsId, status: 20 });
      result = result.data[0] || {};
      this.rasonDialog.remarks = result.auditReason;
      this.rasonDialog.visible = true;
    },
    // 矿发煤价
    onModifyPrice(row) {
      this.$router.push({ path: '/goods/mine/price/' + row.goodsId });
    },
